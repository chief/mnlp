...
describe Mnlp::Automata::Nfsa do
  ...
  describe "#create_transition" do
    ...
    context "when there is the same symbol to another state" do
      ...
      it "creates a new transition" do
        expect { subject.create_transition(0, 2, "A") }.to change{
          subject.transitions.size}.by(1)
  ...
  describe "#recognize!" do
    ...
    context "when a a fuzzy transition happens" do
      before do
        subject.add_state
        subject.create_transition 1, 3, "h"
        subject.add_state
        subject.create_transition 3, 4, "f"
      ...
      it "accepts 'chf' correctly" do
        # binding.pry
        expect(subject.current_state).to be_final
      end
    end
  end
